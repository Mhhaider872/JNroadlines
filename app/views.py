...
def ashland_bill(request):
    ...
    if request.method == "POST":
        ...
        sac = request.POST.get('sac')
        charges_tank = request.POST.get('charges_tank')
        hsac = request.POST.get('hsac')
        ...
        # Step 3: Create empty invoice
         invoice = AInvoice.objects.create(
            ...
            lr_no=lr_no,
            sac=sac if sac else 0,
            charges_tank=charges_tank if charges_tank else None,
            hsac=hsac if hsac else 0,
...
def murli_bill(request):
    if request.method == "POST":
        ...
        hsac = request.POST.get('hsac')
         ...
         # Step 2: Generate invoice number
        invoice_number = generate_bill_no()
        if MTInvoice.objects.filter(date=date,company=company).exists():
            messages.error(request, 'Bill already Exists!!')
            return redirect('murli_bill')
        else:
          
        # Step 3: Create empty invoice
         invoice = MTInvoice.objects.create(
            invoice_number=invoice_number,
            date=date,
            company=company,
            gst=gst,
...
def murli_list(request):
    ginvoice = MTInvoice.objects.all().order_by('date')  # Latest invoice first
    return render(request, 'bills/MURLIWALA/murli_list.html', {'ginvoice': ginvoice})


def murli_detail(request, invoice_id):
    invoice = get_object_or_404(MTInvoice, id=invoice_id)
    items = MTItem.objects.filter(invoice=invoice)
